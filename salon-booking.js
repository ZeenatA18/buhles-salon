module.exports = function salonBooking(db) {
    ...
    async function findAllBookings(date) {
        var db_results = await db.manyOrNone('select client_id, treatment_id,stylist_id, booking_date::text, booking_time from booking where booking_date = $1', [date])
        return db_results
    }

    async function findClientBookings(clientId) {
        var db_results = await db.manyOrNone('select client_id, treatment_id,stylist_id, booking_date::text, booking_time from booking where client_id = $1', [clientId])
        return db_results
    }

    async function findAllBookingz(date, time){
        var db_results = await db.manyOrNone('select client_id, treatment_id,stylist_id, booking_date::text, booking_time from booking where booking_date = $1 AND booking_time = $2', [date, time])
        return db_results
    }
